...
// newSNSClient builds an AWS SDK client from a Config. It never dials the
...
// tag.
func TestNewSNSClient_UsesConfiguredRegion(t *testing.T) {
	cfg := config.Config{AWSRegion: "eu-west-1"}

	client, err := newSNSClient(context.Background(), cfg)
...
func TestNewSNSClient_HonorsCustomEndpoint(t *testing.T) {
...
func TestNewSNSClient_NoEndpointOverride(t *testing.T) {
